...
Below is the cost function for Logistic Regression. Note the $\frac{1}{2}$ has been moved inside the summation.
...
It is non-liner because of the *Sigmoid Function* $ \frac{1}{1+e^-z}$. When using *Gradient Decent* using this cost function you are not guaranteed to converge at the global minimum. What we need is a convex cost function with a single global minimum.
